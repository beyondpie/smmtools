...
class Fragment(object):
    ...
    def __init__(
        self, qname, chrom, pos, flen, mapq, is_single, is_secondary, is_proper_pair
    ):
        """Return a qc object"""
...
def handle_reads_from_one_barcode(
    read_group,
    barcodes: Dict[str, Any],
    min_mapq: int = 30,
    min_flen: int = 0,
    max_flen: int = 1000,
) -> Tuple[collections.Counter, QC]:
    frag_list: List[Tuple[str, int, int]] = []
    qc = QC()
    for (read1, read2, is_paired, is_secondary) in pairReadsByName(
    ...
    return (frag_counter, qc)


def SnapToolsBamTo10xFragmentBed(
    bam_file: str,
    outf: str,
    qc_file: str,
    min_mapq: int = 30,
    ...
    NOTE:
    - need ~bedtools~ in the env
    - bam file should be sorted by name firstly.
    """
    ...
    with open(outf, "w") as fbed, open(qc_file, "w") as fqc:
        fqc.write("{}\n".format(sep.join(QC.fields)))
        for read_group in group_reads_by_barcode_bam(input_bam=bam_file):
            ##  reads from read_group come from the same barcode
            frag_counter, qc = handle_reads_from_one_barcode(
                read_group=read_group,
                barcodes=barcode_cov,
                min_mapq=min_mapq,
                min_flen=min_flen,
                max_flen=max_flen,
            )
            ...
            check_point += 1
            if verbose and check_point % step == 0:
                print(
                    f"{check_point} barcodes, {round(time.time() - start_time, 1)} seconds."
                )
            if len(frag_counter) < 1:
                continue
            n_uniq_chrM = sum([e[0] == "chrM" for e in frag_counter.keys()])
            if n_uniq_chrM == qc.uniq:
                continue
            ## output the fragments
            frag_content: List[str] = [
                f"{k[0]}\t{k[1]}\t{k[2]}\t{qc.barcode}\t{v}"
